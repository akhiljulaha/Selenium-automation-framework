package com.qa.opencart.utils; //4 util	                  //3
...
/*
 * all methods should always non static and if you make static then the variable should also be static then we will 
 * face parllel execution problem or other also can acccess our methods 
 * 
 *  we making private because we dont't want someone is access out of this class, suppose variable is public and 
 *  if someone is trying to accsess then default value of driver is null, might get a NULLPOINTEREXCEPTION
 *  to avoid this Unnessesary Null exception we making it private 
 *  that's why we using Encapsulation concept(Private variable can be access by the public methods)
 */
	private WebDriver driver; // SI ---> 123(c)  
	private final int DEFAULT_TIME_OUT = 5;
	private JavaScriptUtil jsUtil;

	public ElementUtil(WebDriver driver) { // SI --> 123(a) // Note: only with the help of constructor we supplyin driver to all methods
		this.driver = driver; // SI --> 123 (b)
		jsUtil = new JavaScriptUtil(this.driver);        // we writing here because we have to pass the driver 
	}
...
//}
	...
	/*
	 * issue with upper method -> whenever we calling the uper method every time we have to pass wait also Even if
	 * you don't want to use.(Like in the implicitly)
	 * That's why we have to overload the getElement(With wait) method with over standard getElement(Without wait) 
	 *    **want to apply wait for the specific element then call the above method and want to apply
	 *  default wait then call below one.**
	...
	public WebElement getElement(By locator) {
		...
		try {
			...
			element = waitForElementVisble(locator, DEFAULT_TIME_OUT);
		}
...
//		driver.findElement(By.linkText(level4LinkText)).click();
	 ...
	 * Ex>Out of 35 footer if any of the ele is visible on the page and then it will give the comlete list of the footer
	 * @param locator
	 * @param timeOut
	 * @return
	 */
