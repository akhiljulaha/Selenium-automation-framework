...
public class ExcelUtil {
	...
	private static Sheet sheet; // import ss
	...
	public static Object[][] getTextData(String sheetName) {
		...
		try {
			FileInputStream ip = new FileInputStream(TEST_DATA_SHEET_PATH);
				book = WorkbookFactory.create(ip); // it will help to interact with sheet
				sheet = book.getSheet(sheetName);
				data = new Object[sheet.getLastRowNum()][sheet.getRow(0).getLastCellNum()];
				for (int i = 0; i < sheet.getLastRowNum(); i++) {            //4
					for (int j = 0; j < sheet.getRow(0).getLastCellNum(); j++) {         //0,1,2,3,4,5<5
						data[i][j] = sheet.getRow(i + 1).getCell(j).toString();  // converting cell value to the string //ddd?????? 
					}
				}
